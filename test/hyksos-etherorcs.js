...
contract("HyksosEtherorcs test", async () => {
  ...
  it("Simple Orc deposit", async () => {
    ...
    // transfer Orc to account 2, to avoid confusion
    ...
    // verify there's a correct amount of zug left in Hyksos
  ...
  it("Complex Orc deposits", async () => {
    ...
    // 976 / 32 = 30 more Orc deposits should be possible
    ...
    // 31st deposit shouldn't be possible, (16 zug left in Hyksos)
    ...
    // Orc deposit should now be possible
    await orcs.approve(hyksos.address, 0,  {from: accounts[2]});
    await hyksos.depositNft(0, {from: accounts[2], gas: 1e6})
    
    // withdraw Orc and verify that only accounts 3, 4, 5, 6 have received at least 5 zug in rewards
    web3.evm.increaseTime(10 * 86400 + 1);
    await hyksos.withdrawNft(0, {from: accounts[2], gas: 1e6})
    for (let i = 3; i < 7; i++) {
    ...
    // Withdraw zug of account 9 to make a gap in deposit queue. Send them to account 1
    ...
    // Deposit Orc once again and verify that addresses 7, 8, 10, 11, 12, 13, 14, 15 received rewards (31st deposit)
    await orcs.approve(hyksos.address, 0,  {from: accounts[2]});
    await hyksos.depositNft(0, {from: accounts[2], gas: 1e6})
    web3.evm.increaseTime(10 * 86400 + 1);
    await hyksos.withdrawNft(0, {from: accounts[2], gas: 1e6})
    ...
    // verify that no account has received a reward too big
    for (let i = 3; i < 20; i++) {
    ...
    // verify that account 2 has received 32 * 14 zug in total
  ...
  it("Late Orc withdrawal", async () => {
    ...
    // Transfer 16 more zug from account 1 to 19, then deposit them into the pool.
  ...
  it("Auto compounding", async () => {
    ...
    // Deposit 32 zug from account 1 to pool.
    ...
    // Deposit and withdraw Orc 8 times in a row.
    for (let i = 1; i <= 8; i++) {
      await orcs.approve(hyksos.address, 0,  {from: accounts[2]});
      await hyksos.depositNft(0, {from: accounts[2], gas: 1e6});
      web3.evm.increaseTime(10 * 86400 + 1);
